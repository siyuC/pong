...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate, tau):
        ...
        self.a_dim = action_dim  # 6 for pong
        self.learning_rate = learning_rate
        self.tau = tau

        # Actor Network
        self.inputs, self.out = self.create_actor_network() # , self.scaled_out 

        self.network_params = tf.trainable_variables()

        # Target Network
        self.target_inputs, self.target_out = self.create_actor_network() #, self.target_scaled_out 
...
def train(sess, env, actor, critic):
    ...
    for i in xrange(MAX_EPISODES):
        ...
        for j in xrange(MAX_EP_STEPS):
            ...
            # begin training when the batch size is reached.
            if replay_buffer.size() > MINIBATCH_SIZE:     
                ...
                for k in xrange(MINIBATCH_SIZE):
                    if t_batch[k]: # check if it is a terminal state
