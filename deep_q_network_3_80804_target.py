...
def createNetwork():
    ...
    # input layer
    s = tf.placeholder("float", [None, 80, 80, 4],name = 's')
...
def trainNetwork(s, readout, h_fc1, network_params, s_target, readout_target, h_fc1_target, target_network_params,sess):
    ...
    checkpoint = tf.train.get_checkpoint_state("dqn_pong_target")
    ...
    update_target_network_params = [target_network_params[i].assign(network_params[i])
    ...
    while 1:
        ...
        x_t1_col, r_t, terminal, _  = returns
            ...
            if t % 100 == 0:
                ...
                update_target_network_params = \
                        [target_network_params[i].assign(tf.mul(network_params[i], tau) + \
                            ...
                            for i in range(len(target_network_params))]
                sess.run(update_target_network_params)
                # print 'net',network_params[0][0].eval(),'target',target_network_params[0][0][0].eval()
        ...
        if t % 10000 == 0:
            saver.save(sess, 'dqn_pong_target/', global_step = t)
        # print info
        if (t % 1000 == 0):
            print "TIMESTEP", t, "/ REWARD", r_t, "/ EPSILON", epsilon, "/ Q_MAX %e" % np.max(readout_t)
...
def playGame():
    with tf.device('/gpu:6'):
        ...
        trainNetwork(s, readout, h_fc1, network_params, s_target, readout_target, h_fc1_target, target_network_params, sess)
