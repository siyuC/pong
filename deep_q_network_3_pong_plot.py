...
def createNetwork():
    ...
    with tf.name_scope('input'):
        s = tf.placeholder("float", [None, 80, 80, 1],name = 's')
...
def trainNetwork(s, readout, h_fc1, s_target, readout_target, h_fc1_target, sess):
    ...
    checkpoint = tf.train.get_checkpoint_state("dqn_pong")
    ...
    med_t = prepro(env.reset())
    s_t = np.zeros_like(med_t)
    ...
    while 1:
        ...
        if t > 1000:
            ...
            if t % 1000 == 0:
                sess.run(train_step_target,feed_dict = {
                    y : y_batch,
                    a : a_batch,
                    s_target : s_j_batch})
        ...
        # create a summary for rewards
        tf.summary.scalar("rewards", tot_reward)
        summary_op = tf.merge_all_summaries()

        # save progress every 10000 iterations
        if t % 10000 == 0:
            saver.save(sess, 'dqn_pong/', global_step = t)
        # print info
        if (t % 1000 == 0):
...
def playGame():
    ...
    trainNetwork(s, readout, h_fc1, s_target, readout_target, h_fc1_target, sess)
